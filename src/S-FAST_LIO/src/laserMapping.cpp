...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    ...
    sensor_msgs::Imu::Ptr msg(new sensor_msgs::Imu(*msg_in)); // 深拷贝，并且放在堆里，自行管理生命周期

    // 如果启用了软件时间同步，并且last_timestamp_lidar-last_timestamp_imu有差异，就尝试在线地根据上次的数据时间偏差来修正当前IMU的时间戳
    ...
    // 如果用户没有输入时间差，默认为0.0，即不会修正IMU的时间戳
    msg->header.stamp = ros::Time().fromSec(msg_in->header.stamp.toSec() - time_diff_lidar_to_imu);

    double timestamp = msg->header.stamp.toSec();

    mtx_buffer.lock(); // 对共享资源imu_buffer加锁
    ...
    mtx_buffer.unlock();
    sig_buffer.notify_all(); // 我们经常会在互斥锁解锁后，手动通知一下在buffer队列中阻塞的线程，这样能够确保他们（更快地）开始工作
}

double lidar_mean_scantime = 0.0; // 似乎是滑动平均估计每次push，需要Lidar scan的扫描时间
int scan_num = 0;

/**
 * @brief 从buffer中取出当前要处理的LIDAR和IMU数据，打包到meas中
 * @note 有一个线程负责接收数据，又有另一个线程需要处理数据，自然需要一个buffer，本代码使用的是FIFO，常见的还有循环缓冲区
 * @note 同时，还设置了一个time_buffer，存储每个lidar点云的时间戳
 */
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    // 从 imu_buffer 中弹出所有时间戳 < lidar_end_time 的IMU数据，存入 meas.imu
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (flg_exit) // 如果接收到退出信号，退出程序
            break;
        ros::spinOnce(); // 处理回调，包括点云数据和IMU数据

        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            double t00 = omp_get_wtime(); // OpenMP提供的调用时间函数
