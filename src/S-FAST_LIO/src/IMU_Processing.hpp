...
class ImuProcess
{
 public:
  ...
  void Reset();
  /**
  * @brief 传入外部参数，这些参数都应该由用户提供
  * @param transl 平移向量 lidar w.r.t. imu
  * @param rot 旋转矩阵 lidar w.r.t. imu
  * @param gyr 角速度协方差
  * @param acc 加速度协方差
  * @param gyr_bias 角速度零偏
  * @param acc_bias 加速度零偏
  */
...
/**
 * @brief 利用开始的IMU帧的平均值初始化状态量x
 * @param meas 输入测量值
 * @param kf_state 当前esekf状态
 * @param N 迭代次数
 * @note 初始化重力、陀螺仪偏差、acc和陀螺仪协方差  将加速度测量值归一化为单位重力
 */
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)
{
  ...
  init_state.grav = - mean_acc / mean_acc.norm() * G_m_s2;    // 静止初始化，得到重力方向
  init_state.bg  = mean_gyr;      // 静止初始化，角速度测量作为陀螺仪零偏
...
/**
 * @brief 去除点云畸变，对应论文中反向传播过程
 * @param meas 输入测量值
 * @param kf_state 当前esekf状态
 * @param pcl_out 输出去畸变后的点云
 */
